(()=>{
  ...
  function sync(){
    const hp=g('hpInput'); if(hp&&g('mhHp')){const temp=g('tempHpText')?.textContent?.match(/\d+/)?.[0];g('mhHp').textContent=(hp.value||'0')+(temp?' +'+temp:'');}
    ...
    [['ecoAction','mhAction'],['ecoBonus','mhBonus'],['ecoReaction','mhReaction'],['ecoMovement','mhMovement']].forEach(([src,dst])=>{
  ...
  window.addEventListener('DOMContentLoaded',()=>{
    ['slots','turnN','turnDamage','concName','tempHpText','ecoAction','ecoBonus','ecoReaction','ecoMovement'].forEach(id=>{const e=g(id);if(e)observer.observe(e,{subtree:true,childList:true,characterData:true,attributes:true});});
